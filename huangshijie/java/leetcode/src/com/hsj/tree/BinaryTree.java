...
public class BinaryTree<E extends Comparable<E>> {
     ...
     * @param e
     */
    public void addsimplify(E e) {
        root = addsimplify(root, e);
    }
     ...
     * @return
     */
    private Node addsimplify(Node node, E e) {
        if (node == null) {
            size++;
            return new Node(e);
        }
        if (e.compareTo(node.e) < 0) {
            node.leftnode = addsimplify(node.leftnode, e);
        } else {
            if (e.compareTo(node.e) > 0) {
                node.rightnode = addsimplify(node.rightnode, e);
            }
        }
        return node;
    }
    ...
    private Node remove(Node node, E e) {
        ...
        if (e.equals(node.e)){
            ...
            //得到以node.rightnode为根的最小的节点
            Node souccer = minmum(node.rightnode);
